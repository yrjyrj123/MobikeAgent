...
class KML():
    ...
    def add_bike(self,bike):
        ...
        name.appendChild(self.__root.createTextNode(bike['distId']))
        placemark.appendChild(name)

        styleUrl = self.__root.createElement("styleUrl")
        styleUrl.appendChild(self.__root.createTextNode('lite' if bike['biketype']==2 else 'classical'))
        placemark.appendChild(styleUrl)

        point=self.__root.createElement("Point")
        placemark.appendChild(point)
        coordinates=self.__root.createElement("coordinates")
        coordinates.appendChild(self.__root.createTextNode("%s,%s,%s"%(bike['distX'],bike['distY'],"0")))
